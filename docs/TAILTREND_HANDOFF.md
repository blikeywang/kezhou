...
| `BREAKOUT_CANDIDATE` | 突破候选 | 等待后续日线接受，不追完整仓位 |
...
| 突破缓冲 | 旧边界外 0.25 ATR |
| 趋势接受 | 3 日窗口中至少 2 个收盘留在冻结的“旧高点 + 0.25 锁定 ATR”外 |
...
下沿进入下落状态时会冻结实际信号边界 `lowerTailTop`、结构低点和当时 ATR。当天创新结构低点不得被同日反弹误记为收复；结构低点至少稳定 5 个交易日后才允许收复。收盘跌破冻结结构低点 1 ATR 时重建，锁定超过 40 个交易日也会确定性重算。上沿突破同样冻结旧高点与 ATR；趋势接受后清除上沿锁，并持续跟踪进入当天前已知的 10 日低点。

引擎并行记录 `1.5 ATR` 尾部宽度的对照状态，但该字段只用于影子比较，页面正式动作仍以 60 日区间 20% 口径为准。
...
- 高点回撤、HV 百分位和周线环境属于同源压力组，分别计算后只取最严值，不再相乘；
- 普通组合计划风险 1.25% 与同一集群 0.60% 按剩余额度裁剪，不作为第二组乘数；
- 组合 1.50% 与集群 0.75% 仍是硬阻断；
- 单日两次完整止损或当日已实现亏损达到权益 1%，停止新增风险；
- 流动性上限按 20 日平均成交额的 1% 约束；
- 输出 `riskDiagnostics` 和 `bindingConstraint`，明确究竟是哪一道约束卡住仓位。

已知事件进入 10 个日历日风险窗口时，网页把默认跳空准备金基准放大 2.5 倍；进入事件隔离后继续禁止普通 ATR 新仓公式。系统不读取持仓也不发订单，因此不会自动执行事件前减仓。
...
- 分批读取财报日历；
- 并发、超时与最多 3 次重试；
- 每个交易日写入不可变的 `snapshots/YYYY-MM-DD.json`，同日重跑不能覆盖；
- `dataAsOf` 与 `runAt` 分离，并记录引擎 commit、源码哈希和参数哈希；
- 单个标的刷新失败可在尝试阶段读取上一派生状态，但正式日记录写为 `MISSING`；页面只展示上一完整快照的 STALE 副本并强制禁止新仓；
- `index.json` 永久保留每日健康度、桶计数和迁移摘要，不再使用 90 次滚动窗口；
- 只输出派生状态，不输出原始完整 K 线。

目前 Longbridge 刷新必须在已认证的可信电脑手动执行。GitHub Actions 不持有 Longbridge 交易凭据。

1/3/5/10 日 MFE/MAE 继续写入独立的 `tailtrend-audit.json`，允许派生结果随未来交易日更新，但不可变状态快照本身永不回写。
...
- 日快照改成 append-only，新增 `snapshots/`、`latest.json`、`index.json`、版本哈希和显式 `MISSING` 记录；
- 下沿冻结实际信号边界并加入 5 日稳定、1 ATR 重置与 40 日过期规则；
- 上沿接受改为对冻结缓冲线判断，伸展使用锁定 ATR，趋势接受状态持久到此前完成的 10 日低点退出；
- 引擎可接上一日 `stateMemory` 递推，也能冷回放确定性重建；
- 压力风险不再全连乘，组合/集群/流动性改用剩余额度裁剪，并显示 `bindingConstraint`；
- 并行记录 ATR 尾部对照状态；事件窗口采用 2.5 倍跳空预算前置；
- 静态构建写入无 JS 表格回退；本地 K 线导入增加 90 根、5 MB 与 turnover 降级说明；
- 风险表单可导出/导入用户自管的本地配置文件，仍不写浏览器存储。

逐项接受、修正和暂缓的理由见 [`TAILTREND_REVIEW_RESPONSE_V03.md`](TAILTREND_REVIEW_RESPONSE_V03.md)。
...
这份快照是在 append-only 与冻结边界落地前产生的，只保留为开发对照，**不计入 v0.3 的 10 日影子样本**。v0.3 第一份不可变 `COMPLETE` 文件才是新的计天起点。
...
append-only、冻结边界和风险重构完成后，重新连续运行 10 个完整交易日，不立即优化参数。这十天只验证工程一致性，不据此声称统计有效。每天保留下列审计字段：
...
1. 建立 2–3 年可复现的逐日历史状态回放；
2. 用同一回放数据结构生成历史状态时间线和每日差异报告；
